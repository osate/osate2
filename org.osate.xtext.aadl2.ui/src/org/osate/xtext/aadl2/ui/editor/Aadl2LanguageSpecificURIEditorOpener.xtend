...
import static org.osate.xtext.aadl2.ui.editor.Aadl2LanguageSpecificURIEditorOpener.*
	...
	override open(URI uri, EReference crossReference, int indexInList, boolean select) {
		...
		if (storages != null && storages.hasNext) {
			try {
				...
				val selectAndRevealJob = new Job("Select and reveal referenced object") {
					override protected run(IProgressMonitor monitor) {
						selectAndReveal(editor, uri, crossReference, indexInList, select)
						Status.OK_STATUS
					}
				}
				selectAndRevealJob.system = true
				selectAndRevealJob.priority = Job.SHORT

...
public class FaultTreeExport {
	...
	private static void toXML(WriteToFile report, Event root) {
		...
		if (root.getProbability().compareTo(FaultTreeUtils.BigZero) != 0) {
			IProject project = ResourcesPlugin.getWorkspace().getRoot()
					.getProject(root.eResource().getURI().segment(1));
			String specProb = String.format("%1$." + FaultTreeModel.getPrecision(project) + "e", root.getProbability());

			report.addOutput(" probability=\"" + specProb + "\"");
		}
	...
	private static void toPED(WriteToFile report, Event root) {
		...
		if (root.getType() != EventType.INTERMEDIATE) {
			...
			IProject project = ResourcesPlugin.getWorkspace().getRoot()
					.getProject(root.eResource().getURI().segment(1));
			String specProb = String.format("%1$." + FaultTreeModel.getPrecision(project) + "e", prob);

			report.addOutputNewline(root.getName() + ";;B;" + description + ";" + specProb + ";L;");
			return;
		}

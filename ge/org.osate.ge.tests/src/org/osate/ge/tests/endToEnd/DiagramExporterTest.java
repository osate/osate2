...
public class DiagramExporterTest {
	...
	public void testExporter() {
		...
		final DiagramExportService diagramExportService = Objects.requireNonNull(
				PlatformUI.getWorkbench().getService(DiagramExportService.class),
				"Unable to get diagram exports service");

		// Must run in an UI thread
		final AtomicReference<RuntimeException> exception = new AtomicReference<>();
		Display.getDefault().syncExec(() -> {
			try {
				final IFile diagramFile = (IFile) ResourcesPlugin.getWorkspace()
						.getRoot()
				...
				diagramExportService.export(diagramFile, tmpFile, "png", 1.0);
				assertTrue(tmpFile.exists());

				final ByteArrayOutputStream baos = new ByteArrayOutputStream(48000);
				diagramExportService.export(diagramFile, baos, "svg", 1.0);
				baos.flush();
				...
				diagramExportService.export(diagramFile, mcios, "png", 1.0);
				mcios.flush();
				assertTrue(baos.size() > 0);

...
public class ErrorModelTest {
	...
	public void testErrorModelLibrary() {
		...
		{
			openAadlPropertiesTab(diagram,
					element(pkgRef, ErrorModelReferenceUtil.getRelativeReferenceForTypeSet("ts1")));
	...
	public void testErrorFlows() {
		...
		createElementAndLayout(diagram, subsystemRef, "Propagation Point",
				ErrorModelReferenceUtil.getRelativeReferenceForPropagationPoint("subsystem_impl_new_propagation_point"),
				"ss_pp1");
		...
		final DiagramElementReference sysImplElement = element(pkgRef,
				getClassifierRelativeReference("test_system.impl"));
		...
		//
		// Create and rename propagation point
		createElementAndLayout(diagram, sysImplElement, "Propagation Point", ErrorModelReferenceUtil
				.getRelativeReferenceForPropagationPoint("test_system_impl_new_propagation_point"), "pp1");

		// Create another propagation point and delete it.
		createElementAndLayout(diagram, sysImplElement, "Propagation Point", ErrorModelReferenceUtil
				.getRelativeReferenceForPropagationPoint("test_system_impl_new_propagation_point"), "pp2");
		...
		createElementAndLayout(diagram, pkgElement.join(getClassifierRelativeReference("test_fgt")), "Data Port",
				getFeatureRelativeReference("test_fgt_new_feature"), "i1");
		...
		createShapeElement(diagram, i1Ref, "In Error Propagation",
				ErrorModelReferenceUtil.getRelativeReferenceForPropagation(false, DirectionType.IN),
				ErrorModelTest::selectSingleErrorType);
		final DiagramElementReference i1PropagationRef = i1Ref
				.join(ErrorModelReferenceUtil.getRelativeReferenceForPropagation(false, DirectionType.IN));
		...
		createShapeElement(diagram, pp1Ref, "Out Error Containment",
		...
		// Propagation Path
		//
		createConnectionAndLayout(diagram, o1Ref, sc1Pp1Ref, "Propagation Path", sysImplElement,
				ErrorModelReferenceUtil.getRelativeReferenceForPropagationPath("test_system_impl_new_propagation_path"),
		...
		// Set the fault condition
		{
			...
			waitForDiagramElementCondition(diagram, errorSourceElement,
					"Fault condition does not match '" + faultCondition + "'", de -> {

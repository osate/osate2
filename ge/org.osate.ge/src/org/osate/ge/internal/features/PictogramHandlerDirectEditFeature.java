...
// Direct Editing Feature implementation that delegates behavior to a pictogram handler
public class PictogramHandlerDirectEditFeature extends AbstractDirectEditingFeature implements ICustomUndoRedoFeature {
	...
	@Inject
	public PictogramHandlerDirectEditFeature(final IFeatureProvider fp, final ExtensionService extService,
    ...
    public boolean canDirectEdit(final IDirectEditingContext context) {
		...
		if(!PictogramHandlerUpdateFeature.nameShapeName.equals(propertyService.getName(context.getPictogramElement()))) {
			return false;
		}
		
		// Find pictogram handler that handles the business object
		final Object handler = extService.getApplicablePictogramHandler(bo);
		if(handler == null) {
			return false;
		}

...
class FilterByBusinessObjectCanonicalReferenceQuery<T> extends DefaultQuery<T> {
	...
	public FilterByBusinessObjectCanonicalReferenceQuery(final DefaultQuery<T> prev,
	...
	void run(final Deque<DefaultQuery<T>> remainingQueries, final BusinessObjectContext ctx,
		...
		final CanonicalBusinessObjectReference boRef = (CanonicalBusinessObjectReference) state.cache
				.computeIfAbsent(this, t -> {
					final Object bo = boSupplier.apply(state.arg);
					CanonicalBusinessObjectReference newValue = bo == null ? nullBoRef
							: state.refBuilder.getCanonicalReference(bo);
					if (newValue == null) {
						newValue = nullBoRef;
					}
					return newValue;

...
public class CreateDiagramHandler extends AbstractHandler {
	...
	public Object execute(final ExecutionEvent event) throws ExecutionException {
		try {
			...
			final Object contextBo = SelectionUtil.getDiagramContext(AgeHandlerUtil.getCurrentSelection(),
					HandlerUtil.getActiveEditor(event));
			if (contextBo == null) {
				MessageDialog.openError(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(),
						"Unable to Create Diagram", "Selection is not a valid diagram context.");
				return null;
			}
	...
	public void setEnabled(final Object evaluationContext) {
		...
		setBaseEnabled(SelectionUtil.isAadlOrInstanceModelFile(selection) || SelectionUtil.getDiagramContext(
				selection,
				AgeHandlerUtil.getActiveEditorFromContext(evaluationContext)) != null);
	}
}

...
class ShowHideFiltersContributionItem extends CompoundContributionItem {
	...
	private void addContributionItems(final List<IContributionItem> contributions, final String parentId,
		...
		.sorted((cf1, cf2) -> cf1.getName().compareToIgnoreCase(cf2.getName())).forEachOrdered(filter -> {
			...
			final CommandContributionItemParameter param = new CommandContributionItemParameter(window,
					null, commandId, CommandContributionItem.STYLE_PUSH);
			param.parameters = Collections.singletonMap(filterParameterId, filter.getId());
			param.visibleEnabled = true;

			if (childFilters.size() == 0) {
				param.label = filter.getName();
				final CommandContributionItem commandItem = new CommandContributionItem(param);

...
public class ShowConnectedElementsHandler extends AbstractHandler {
	...
	public Object execute(final ExecutionEvent event) throws ExecutionException {
		...
		final List<BusinessObjectContext> selectedElements = AgeHandlerUtil.getSelectedBusinessObjectContexts().stream()
				.filter(boc -> isConnectionEnd(boc))
				.collect(Collectors.toList());

		for (final BusinessObjectContext selectedElement : selectedElements) {
			// ConnectionEnd or ConnectionInstanceEnd
			final ConnectionEnd connectionEnd = (ConnectionEnd) selectedElement.getBusinessObject();
			System.err.println(connectionEnd + " connectionEnd");
		}
	...
	private static boolean isConnectionEnd(final BusinessObjectContext boc) {
		final Object bo = boc.getBusinessObject();
		return !(ConnectionEnd.class.isInstance(bo) || ConnectionInstanceEnd.class.isInstance(bo));
	}

...
public class PackageSetExtendedClassifierFeature extends AbstractCustomFeature {
	...
	@Inject
	public PackageSetExtendedClassifierFeature(final IFeatureProvider fp, final BusinessObjectResolutionService bor,
	...
	public void execute(final ICustomContext context) {
		final Classifier classifier = (Classifier) bor.getBusinessObjectForPictogramElement(context
		...
		modificationService.modify(classifier, new AbstractModifier<Classifier, Object>() {
			...
			public Object modify(final Resource resource, final Classifier classifier) {
				...
				final Classifier selectedClassifier = (dlg.getFirstSelectedElement() != null && ((EObject) dlg
						.getFirstSelectedElement()).eIsProxy()) ? (Classifier) EcoreUtil.resolve(
						((EObject) dlg.getFirstSelectedElement()), resource) : (Classifier) dlg
				...
				if (classifier instanceof ComponentType) {
					...
					((ComponentImplementation) classifier).createOwnedExtension().setExtended(
							(ComponentImplementation) selectedClassifier);
				} else if (classifier instanceof FeatureGroupType) {
					((FeatureGroupType) classifier).createOwnedExtension().setExtended(
							(FeatureGroupType) selectedClassifier);
				}
		...
		if (name != null) {
			...
			if (bo instanceof ComponentType) {
				if (bo.eClass() != Aadl2Factory.eINSTANCE.getAadl2Package().getAbstractType()) {
					for (final IEObjectDescription desc : EMFIndexRetrieval
							.getAllClassifiersOfTypeInWorkspace(Aadl2Factory.eINSTANCE.getAadl2Package()
									.getAbstractType())) {
						if (!name.equalsIgnoreCase(desc.getName().toString("::"))) {
							objectDescriptions.add(desc);
						}

...
public class CreateNoteReferencePaletteCommand extends BasePaletteCommand implements CreateConnectionPaletteCommand {
	...
	public Optional<Operation> getOperation(final GetCreateConnectionOperationContext ctx) {
		...
		final DiagramElement destinationDiagramElement = (DiagramElement) ctx.getDestination();

		return Optional.of(Operation.createWithBuilder(createOp -> {
			createOp.supply(() -> StepResultBuilder.create()
					.showNewBusinessObject(ctx.getSource(), new NoteReference(destinationDiagramElement.getId()))
					.build());
		}));
	}

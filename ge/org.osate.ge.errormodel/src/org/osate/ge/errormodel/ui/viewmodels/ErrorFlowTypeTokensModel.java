...
/**
 * {@link TypeTokenListEditorModel} implementation which edits the type tokens of an {@link ErrorFlow} provided by a
 * {@link BusinessObjectSelection}
 *
 */
public class ErrorFlowTypeTokensModel extends BaseTypeSetTypeTokensModel implements TypeTokenListEditorModel {
	 ...
	 */
	public ErrorFlowTypeTokensModel(final BusinessObjectSelection bos) {
		super(true);
	...
	public final void setBusinessObjectSelection(final BusinessObjectSelection value) {
		...
		final IProject project = ProjectUtil.getProjectForBo(bos.boStream(ErrorFlow.class).findAny().orElse(null))
				.orElse(null);

		// Set the type sets used by the model. Create an empty type set object to use in place of null values.
		setState(bos.boStream(ErrorFlow.class)
				.map(f -> f.getTypeTokenConstraint() == null ? ErrorModelFactory.eINSTANCE.createTypeSet()
						: f.getTypeTokenConstraint())
				.collect(Collectors.toList()), project);
	}

	@Override
	protected void modifyTypeSets(final Consumer<TypeSet> modifier) {
		bos.modify(ErrorFlow.class, f -> {
			// Create the type set if it doesn't exist so that there will be a non-null type set to pass to the modifier
			if (f.getTypeTokenConstraint() == null) {
				f.setTypeTokenConstraint(ErrorModelFactory.eINSTANCE.createTypeSet());
			}

			modifier.accept(f.getTypeTokenConstraint());

			// Set the type set to null if it is empty. Type sets are not allowed to be empty.
			if (f.getTypeTokenConstraint().getTypeTokens().isEmpty()) {

		...
		throws IllegalNodeException
		{
			...
			if (sct.isSuperTypeOf(requiredType) || cl.isSuperTypeOf(requiredType) ){
				...
				if (conn instanceof PortConnection) {
					ce=findPortConnectionEnd((PortConnection)context.eContainer(), cxt, s);
				} else if (conn instanceof AccessConnection) {
					ce=findAccessConnectionEnd((AccessConnection)context.eContainer(), cxt, s);
				} else if (conn instanceof FeatureGroupConnection) {
					ce=findFeatureGroupConnectionEnd((FeatureGroupConnection)context.eContainer(), cxt, s);
				} else if (conn instanceof FeatureConnection) {
					ce=findFeatureConnectionEnd((FeatureConnection)context.eContainer(), cxt, s);
				} else if (conn instanceof ParameterConnection) {
					ce=findParameterConnectionEnd((ParameterConnection)context.eContainer(), cxt, s);
				}
				if(ce != null) {
				...
				EObject searchResult = getContainingClassifier(context).findNamedElement(s);
...
//		}
		 ...
		 * TODO: Check for circular dependencies with prototypes.
		 */
//		private  ComponentClassifier findClassifierForComponentPrototype(Classifier topLevelClassifier, List<Subcomponent> subcomponents,
...
//		}
		 ...
		 */
		//TODO: Check for circular dependencies with prototypes.
//		private  FeatureGroupType findFeatureGroupTypeForFeatureGroupPrototype(Classifier topLevelClassifier,
...
//		private  PropertyAssociation getContainingPropertyAssociation(ContainmentPathElement pathElement)
//		{

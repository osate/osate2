...
public class VerificationPlanFileWizard extends AbstractNewFileWizard {
	private final String VERIFICATION_PLAN_LABEL = "Verification plan name";
	private final String REQUIREMENT_SET_LABEL = "Requirement set";
	@Inject
	private IParser parser;
	@Inject
	private VerifyGrammarAccess grammarAccess;

	public VerificationPlanFileWizard() {
		super("Verification Plan", "verify", 1, VerifyActivator.getInstance().getLog(), "org.osate.verify.ui");
	...
	public String fileContents(final Map<String, String> fieldValues) {
		return "verification plan " + fieldValues.get(VERIFICATION_PLAN_LABEL) + " for "
				+ fieldValues.get(REQUIREMENT_SET_LABEL) + " [\n\t\n]";
	}
}

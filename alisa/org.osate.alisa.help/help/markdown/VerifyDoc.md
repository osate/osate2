...
* *Result list*: optional comma separated list of formal parameter specifications to indicate result values. The Result object values will be assigned in order to Computed Variables specified as part of the method call in a verification activity. If the method returns a single value it is assigned to the compute variable. If the returned numeric value has no unit and **in** &lt;unitliteral&gt; is specified the unit literal is attached to the numeric value. If the returned value comes from a Result object its unit string value is converted to a UnitLiteral that is included with the numeric value. In addition if **in** &lt;unitliteral&gt; is specified the value is scaled to the specified unit.

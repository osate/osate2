...
public class CompositionalWeightAnalysis {
   ...
   */
  public double sumSubWeightLimits(final ComponentInstance ci) {
  ...
  public double sum(final List<Double> doublecollection) {
  ...
  public double WeightLimitReserve(final ComponentInstance ci) {
    EList<ComponentInstance> _componentInstances = ci.getComponentInstances();
    ...
    double _sumSubWeightLimits = this.sumSubWeightLimits(ci);
    return (_weightLimit - _sumSubWeightLimits);
  }
  
  public boolean WeightLimitInvariant(final ComponentInstance ci) {
    EList<ComponentInstance> _componentInstances = ci.getComponentInstances();
    boolean _isEmpty = _componentInstances.isEmpty();
    if (_isEmpty) {
      return true;
  ...
  public int subsWithWeightLimit(final ComponentInstance ci) {
    EList<ComponentInstance> _componentInstances = ci.getComponentInstances();
    boolean _isEmpty = _componentInstances.isEmpty();
    if (_isEmpty) {
      return (-1);
  ...
  public boolean assumeWithWeightLimit(final ComponentInstance ci) {
    boolean _xblockexpression = false;
    {
      final int count = this.subsWithWeightLimit(ci);
      if ((count < 0)) {
        return true;
      }
      EList<ComponentInstance> _componentInstances = ci.getComponentInstances();
  ...
  public double sumAllWeightLimits(final ComponentInstance ci) {

...
class PlatformResourceBudgets extends DefaultVerificationMethodDispatcher{
...
//	@For SCS.tier0 == prove declaration == for in ReqSpec 
	...
	def boolean validateSubBudgetPrecondition(NamedElement ne){
		...
		def dispatchVerificationMethod(String methodPath, ComponentInstance target) {
		switch (methodPath){
			case "org.osate.verify.PlatformResourceBudsgets.assertSumSubBudgets": {
				target.assertSumSubBudgets
			}
			case "org.osate.verify.PlatformResourceBudsgets.sumSubBudgets": {

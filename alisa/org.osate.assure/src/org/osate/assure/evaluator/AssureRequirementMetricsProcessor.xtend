...
class AssureRequirementMetricsProcessor implements IAssureRequirementMetricsProcessor{
	...
	def dispatch void process(ModelResult modelResult) {
		...
		modelResult.metrics.qualityCategoryRequirementsCount = targetReqs.map[content.filter[!(targetElement instanceof ClassifierFeature)]].flatten.map[qualityCategory].flatten.toSet.size

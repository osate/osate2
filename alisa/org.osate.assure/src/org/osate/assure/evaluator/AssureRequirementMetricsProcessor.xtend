...
class AssureRequirementMetricsProcessor implements IAssureRequirementMetricsProcessor{
	...
	def dispatch void process(ModelResult modelResult) {
		...
		modelResult.metrics.featuresRequirementsCount = sysReqs.map[content].flatten.map[targetElement].filter(ClassifierFeature).size		
	...
	def dispatch void process(SubsystemResult caseResult) {
		...
		caseResult.metrics.featuresRequirementsCount = sysReqs.map[content].flatten.map[targetElement].filter(ClassifierFeature).size	

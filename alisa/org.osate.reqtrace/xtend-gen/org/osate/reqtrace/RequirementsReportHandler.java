...
public class RequirementsReportHandler extends AbstractHandler {
  ...
  private static final HashMap<String, String> EMITTERS = CollectionLiterals.<String, String>newHashMap(
    ((Pair<? extends String, ? extends String>[])Conversions.unwrapArray(IterableExtensions.<Pair<String, String>>filter(Iterables.<Pair<String, String>>concat(ListExtensions.<IExtension, Iterable<Pair<String, String>>>map(((List<IExtension>)Conversions.doWrapArray(Platform.getExtensionRegistry().getExtensionPoint("org.eclipse.birt.report.engine.emitters").getExtensions())), 
      ...
      boolean _contains = RequirementsReportHandler.EMITTERS_TO_IGNORE.contains(it.getValue());
  ...
  public Object execute(final ExecutionEvent event) throws ExecutionException {
    ...
    {
      IPath path = null;
      Pair<?, String> _switchResult = null;
      ...
      if (selectedObject instanceof IFile) {
        _matched=true;
        Pair<IPath, String> _xblockexpression_1 = null;
        {
          path = ((IFile)selectedObject).getFullPath();
          IPath _fullPath = ((IFile)selectedObject).getFullPath();
          String _fileExtension = ((IFile)selectedObject).getFileExtension();
          _xblockexpression_1 = Pair.<IPath, String>of(_fullPath, _fileExtension);
        }
        _switchResult = _xblockexpression_1;
      }
      if (!_matched) {
        if (selectedObject instanceof ComponentClassifier) {
          _matched=true;
          _switchResult = Pair.<ComponentClassifier, String>of(((ComponentClassifier)selectedObject), "Classifier");
        }
      }
      if (!_matched) {
        if (selectedObject instanceof EObjectURIWrapper) {
          _matched=true;
          Pair<ComponentClassifier, String> _xblockexpression_1 = null;
          {
            EObject _eObject = new ResourceSetImpl().getEObject(((EObjectURIWrapper)selectedObject).getUri(), true);
            final ComponentClassifier eObject = ((ComponentClassifier) _eObject);
            _xblockexpression_1 = Pair.<ComponentClassifier, String>of(eObject, "Classifier");
          }
          _switchResult = _xblockexpression_1;
        }
      }
      final Pair<?, String> selectedObjectAndType = _switchResult;
      Shell _activeShell = HandlerUtil.getActiveShell(event);
      List<String> _sort = IterableExtensions.<String>sort(RequirementsReportHandler.EMITTERS.keySet());
      final RequirementsReportConfigDialog dialog = new RequirementsReportConfigDialog(_activeShell, _sort);
      int _open = dialog.open();
      boolean _equals = (_open == Window.OK);
      if (_equals) {
        final IPath selectionPath = path;
          ...
          IStatus _xblockexpression_1 = null;
          {
            ...
            final Procedure1<IReportEngine> _function_2 = (IReportEngine it_1) -> {
              try {
                ...
                _builder.append("/requirements.rptdesign");
                ...
                final Procedure1<IRunAndRenderTask> _function_3 = (IRunAndRenderTask it_2) -> {
                  try {
                    ...
                    it_2.setParameterValue("selectedObject", selectedObjectAndType.getKey());
                    it_2.setParameterValue("selectionType", selectedObjectAndType.getValue());
                    ...
                    final Procedure1<RenderOption> _function_4 = (RenderOption it_3) -> {
                      ...
                      it_3.setEmitterID(RequirementsReportHandler.EMITTERS.get(file.substring(_plus)));

...
public class InterpreterUtil {
	...
	public static int compareTo(final NumberValue v1, final NumberValue v2) {
		int _xblockexpression = 0;
		{
			...
			if (_and) {
				...
				int _xblockexpression_2 = 0;
				{
					double _xifexpression_1 = 0;
					UnitLiteral _unit_2 = v1.getUnit();
					boolean _equals = Objects.equal(_unit_2, null);
					if (_equals) {
						double _xifexpression_2 = 0;
						if ((v1 instanceof IntegerLiteral)) {
					...
					double _xifexpression_3 = 0;
					UnitLiteral _unit_3 = v2.getUnit();
					boolean _equals_1 = Objects.equal(_unit_3, null);
					if (_equals_1) {
						double _xifexpression_4 = 0;
	 ...
	 */
	public static InstanceObject resolve(final AModelReference ref, final InstanceObject root) {
		InstanceObject _xifexpression = null;
		AModelReference _prev = ref.getPrev();
		boolean _equals = Objects.equal(_prev, null);
		if (_equals) {
			_xifexpression = root;
		} else {
			InstanceObject _xblockexpression = null;
			{
				AModelReference _prev_1 = ref.getPrev();
				final InstanceObject io = InterpreterUtil.resolve(_prev_1, root);
				EList<EObject> _eContents = io.eContents();
				final Function1<EObject, Boolean> _function = (EObject it) -> {
					boolean _xifexpression_1 = false;
					if ((it instanceof InstanceObject)) {
						String _name = ((InstanceObject) it).getName();
						NamedElement _modelElement = ref.getModelElement();
						String _name_1 = _modelElement.getName();
						_xifexpression_1 = _name.equalsIgnoreCase(_name_1);
					} else {
						_xifexpression_1 = false;
					}
					return Boolean.valueOf(_xifexpression_1);
				};
				EObject _findFirst = IterableExtensions.<EObject> findFirst(_eContents, _function);
				_xblockexpression = ((InstanceObject) _findFirst);
			}
			_xifexpression = _xblockexpression;
		}
		return _xifexpression;

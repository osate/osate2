...
public class CommonEObjectHoverProvider extends DefaultEObjectHoverProvider {
	@Inject
	private CommonTypeSystem cts;
	@Inject
	private CommonInterpreter inter;
	@Inject
	private CommonStringRepresentation csr;

	@Override
	public String getFirstLine(final EObject o) {
		var result = cts.type(o);
		if (result.failed()) {
			return "no type ";
		}

		var text = csr.string(result.getFirst());
		if (o instanceof ValDeclaration declaration) {
			var value = inter.interpretExpression(declaration.getValue());
			if (!value.failed()) {
				text += value.getValue();
			}
		}
		var label = getLabel(o);
		return text + (label == null ? "" : " <b>" + label + "</b>");
	}
}

...
public final class Dialog {
	...
	public static boolean askQuestion(final String title, final String question) {
		final BooleanAnswer obj = new BooleanAnswer() {
			public void run() {
				this.answer = MessageDialog.openQuestion(getShell(), title, question);
			}
		};
	...
	public static int askQuestion(final String title, final String question,
			final String[] choices, final int defChoice) {
		final IntAnswer obj = new IntAnswer() {
			public void run() {
		        final MessageDialog dialog =
		        	new MessageDialog(getShell(), title, null,
		                question, MessageDialog.QUESTION, 
		                choices, defChoice);
		        this.answer = dialog.open();
			}
		};
		Display.getDefault().syncExec(obj);
		return obj.answer;
	...
	public static String getInput(final String title, final String prompt,
			final String initialValue, final IInputValidator validator) {
		final StringAnswer obj = new StringAnswer() {
			public void run() {
				final InputDialog dialog = 
					new InputDialog(getShell(), title, prompt, initialValue, validator); 

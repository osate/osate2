...
public class SOMChooserDialog extends Dialog
{
	...
	public SOMChooserDialog(final Shell parentShell, final SystemInstance systemInstance,
			final boolean requireName) throws IllegalArgumentException
	{
		super(parentShell);
		...
		if (systemInstance.getSystemOperationModes().size() == 1 &&
				((SystemOperationMode)systemInstance.getSystemOperationModes().get(0)).getName().equals("No Modes"))
	 ...
	 * @throws IllegalArgumentException Thrown if <code>systemInstance</code> is not modal.
	 */
	public SOMChooserDialog(final Shell parentShell, final SystemInstance systemInstance) throws IllegalArgumentException
	{
	 ...
	 */
	public SOMChooserDialog(final Shell parentShell, final SystemInstance systemInstance, final String SOMName, final String SOMPathText)
		throws IllegalArgumentException
	{
		this(parentShell, systemInstance);
		this.newSOMName = SOMName;
		...
		for (int pathIndex = 0; pathIndex < individualModePaths.length; pathIndex++)
		{
			...
			if (modePathElements.length < 2)
				throw new IllegalArgumentException(individualModePaths[pathIndex] + " is not a valid ModeInstance path.");
			...
			{
				final ComponentInstance childComponent = findComponentInstance(parentComponent, modePathElements[elementIndex]);
			...
			{
				final ModeInstance currentMode = findModeInstance(currentComponent, modePathElements[modePathElements.length - 1]);
				if (currentMode != null)
				{
				...
				else if (columnIndex == 1)
				{
					...
					if (componentExistsInCurrentMode(component))
					{
						if (selectedModes.containsKey(component))
						{
							...
							{
								return ((ModeInstance)selectedModes.get(component)).getName();
							}
						}
						else
						{

 ...
 *
 *
 * @version $Id: ResetMarkers.java,v 1.11 2005-09-28 21:24:40 aarong Exp $
 */
...
/**
 * ResetMarkers removes any markers attached to the selected Resource in the Eclipse Navigator.
 */
public class ResetMarkers extends AbstractHandler {
	@Override
	public Object execute(ExecutionEvent event) throws ExecutionException {
		Set<IResource> currentSelection = getCurrentSelection(event);
		if (currentSelection.isEmpty()) {
			MessageDialog.openError(HandlerUtil.getActiveShell(event), "Reset Markers Errror",
		...
		for (final Iterator<IResource> rsrcs = currentSelection.iterator(); rsrcs.hasNext();) {
			...
			try {
				rsrc.deleteMarkers(null, true, IResource.DEPTH_INFINITE);

		...
		if (sourceState.size() == 1) {
			...
			if (false == _alreadyFoundDispatchRelativeTimeoutTransition.containsKey(bs) && bs.isComplete()) {
				...
				if (vl.size() > 0) {
					...
					if (value instanceof NamedValue
						...
						if (literal.equalsIgnoreCase(DispatchTriggerProperties.TIMED)) {
							...
							{
								reportLegalityError(tc,
										"The dispatch relative timeout" + " and catch statement must declared in timed "
				...
				this.reportLegalityError(tc,
						"The dispatch relative timeout and"
				...
				if (false == _alreadyReportedErroneousTransition.contains(bt)) {
					this.reportLegalityError(tc,
							"The dispatch relative timeout and "
									...
									+ "of a complete state: Behavior Annex" + " D.4.(L1) legality rule failed");

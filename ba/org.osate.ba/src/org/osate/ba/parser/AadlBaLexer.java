...
public class AadlBaLexer extends Lexer {
	...
	public static final String[] ruleNames = { "ABS", "AND", "ANY", "BINDING", "CLASSIFIER", "COMPLETE", "COMPUTATION",
			...
			"REM", "STATE", "STATES", "STOP", "TIMEOUT", "TRANSITIONS", "TRUE", "UNTIL", "UPPER_BOUND", "WHILE", "XOR",
			"LBRACK", "RBRACK", "COMMA", "LTRANS", "RTRANS", "LPAREN", "RPAREN", "CONCAT", "LCURLY", "RCURLY", "COLON",
			"ASSIGN", "PROPERTYASSIGN", "EXCLAM", "INTERROG", "GGREATER", "EXCLLESS", "EXCLGREATER", "DOT", "TICK",
			"EQUAL", "NOTEQUAL", "LESSTHAN", "LESSOREQUAL", "GREATERTHAN", "GREATEROREQUAL", "PLUS", "MINUS", "STAR",
			"DIVIDE", "STARSTAR", "DOTDOT", "SEMICOLON", "DOUBLECOLON", "HASH", "ERR_EQUALEQUAL", "ERR_END",
			"ERR_ELSIF", "IDENT", "LETTER", "STRING_LITERAL", "REAL_LIT", "INTEGER_LIT", "DIGIT", "EXPONENT",
			"INT_EXPONENT", "EXTENDED_DIGIT", "BASED_INTEGER", "BASE", "HEX_DIGIT", "WS", "SL_COMMENT", "ESC" };
	...
	public void emit(Token token) {
		...
		if (type != EOF_ID && type != IDENT) {
			...
			{
				_ht.addToHighlighting(_annexOffset, token, AnnexHighlighterPositionAcceptor.KEYWORD_ID);
			} else if (type <= PUNCTUATION_MAX_ID) {
				_ht.addToHighlighting(_annexOffset, token, AnnexHighlighterPositionAcceptor.PUNCTUATION_ID);
			} else if (type <= ERR_MAX_ID) {
				_ht.addToHighlighting(_annexOffset, token, AnnexHighlighterPositionAcceptor.INVALID_TOKEN_ID);
			} else {
				switch (type) {
				case STRING_LITERAL: {
					_ht.addToHighlighting(_annexOffset, token, AnnexHighlighterPositionAcceptor.STRING_ID);
					break;
				}

				case INTEGER_LIT:
				case REAL_LIT: {
					_ht.addToHighlighting(_annexOffset, token, AnnexHighlighterPositionAcceptor.NUMBER_ID);
					break;
				}

				case SL_COMMENT: {
					_ht.addToHighlighting(_annexOffset, token, AnnexHighlighterPositionAcceptor.COMMENT_ID);
					break;
				}
				}
			}
	...
	public static final String _serializedATN = "\3\u0430\ud6d1\u8206\uad2d\u4417\uaef1\u8d80\uaadd\2W\u02d9\b\1\4\2\t"

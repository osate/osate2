...
public class ThreadCheck extends AbstractCheck {
	...
	public void perform(SystemInstance si) {
		 ...
		 */
		final List<ComponentInstance> threadMissingDispatch = allThreads.stream().filter( comp -> 
						        (GetProperties.getDispatchProtocol(comp) == null) &&
						        ((GetProperties.getDispatchProtocol(comp).toString().equalsIgnoreCase ("periodic")) || (GetProperties.getDispatchProtocol(comp).toString().equalsIgnoreCase ("sporadic")))
						        )
		 ...
		 */
		final List<ComponentInstance> threadMissingPeriod = allThreads.stream().filter( comp -> (GetProperties.getPeriodinMS(comp) == 0.0)).collect(Collectors.toList());
		for (ComponentInstance thr : threadMissingPeriod)
		{
			addError (new ErrorReport (thr, "Thread needs to define the Period property"));
		}
		
		final List<ComponentInstance> threadMissingDeadline = allThreads.stream().filter( comp -> (GetProperties.getDeadlineinMilliSec(comp) == 0.0)).collect(Collectors.toList());

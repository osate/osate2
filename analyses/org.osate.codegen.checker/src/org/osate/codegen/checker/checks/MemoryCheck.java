...
public class MemoryCheck extends AbstractCheck {
	...
	public void perform(SystemInstance si) {
		...
		if (pok()) {
			final List<ComponentInstance> segmentsWithoutSize = si.getAllComponentInstances().stream()
					.filter(comp -> (comp.getCategory() == ComponentCategory.MEMORY)
							&& (GetProperties.getByteCount(comp) == 0.0) && (comp.getContainingComponentInstance()
									.getCategory() == ComponentCategory.MEMORY))

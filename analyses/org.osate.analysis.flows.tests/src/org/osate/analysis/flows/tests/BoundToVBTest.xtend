...
class BoundToVBTest extends XtextTest {
	...
	def void BoundToVBTest() {
		...
		assertTrue((resab.values.get(1) as RealValue).value == (4.0))
		assertTrue((resab.values.get(2) as RealValue).value == (18.0))
		assertTrue((resab.values.get(3) as RealValue).value == (4.0))
		assertTrue((resab.values.get(4) as RealValue).value == (8.0))
		assertTrue((resab.values.get(5) as RealValue).value == (3.0))
		assertTrue((resab.values.get(6) as RealValue).value == (16.0))
		resab.subResults.size.assertEquals(3)
		resab.diagnostics.size.assertEquals(3)
		val subres = resab.subResults.get(1)
		val proto = subres.subResults.get(0)
		val subpart1 = proto.sourceReference as NamedElement
		assertTrue(subpart1 instanceof ComponentInstance)
		assertEquals(subpart1.name,"sub3")
		assertTrue(subpart1.isVirtualBus)
		assertTrue(proto.diagnostics.size == 1)
		assertEquals(proto.diagnostics.get(0).message, "Best case 0 ms worst case 10.0ms (period) sampling delay")
	}

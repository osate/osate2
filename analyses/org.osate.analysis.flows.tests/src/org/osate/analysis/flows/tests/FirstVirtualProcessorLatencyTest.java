...
public class FirstVirtualProcessorLatencyTest extends XtextTest {
	private static final String FILE = "org.osate.analysis.flows.tests/models/latencytest/partition3.aadl";

	@Inject
	TestHelper<AadlPackage> testHelper;

	@Test
	public void FirstVPLatencyContribution() throws Exception {
		...
		assertEquals(8, etefResult.getSubResults().size());
		assertEquals(1, etefResult.getDiagnostics().size());

		var firstSubResult = etefResult.getSubResults().get(0);
		assertTrue(firstSubResult.getModelElement() instanceof ComponentInstance);
		assertEquals(2, firstSubResult.getDiagnostics().size());
		assertEquals("Partition not found in ARINC653 schedule of processor sub4",
				firstSubResult.getDiagnostics().get(0).getMessage());
		assertEquals("Initial 1.0ms partition latency not added",
				firstSubResult.getDiagnostics().get(1).getMessage());

		// The partition without an explicit period does not get a Period property association
		var withoutPeriod = etefResult.getSubResults().get(3).getModelElement();
		...
		var withPeriod = etefResult.getSubResults().get(6).getModelElement();

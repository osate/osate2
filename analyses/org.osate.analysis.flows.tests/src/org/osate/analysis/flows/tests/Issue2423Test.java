...
public class Issue2423Test extends XtextTest {
	private static final String FILE = "org.osate.analysis.flows.tests/models/issue2423/Issue2423.aadl";

	@Inject
	TestHelper<AadlPackage> testHelper;

	@Test
	public void issue2423etef() throws Exception {
		...
		var implementation = (SystemImplementation) pkg.getOwnedPublicSection()
				...
				.findFirst()
				.orElseThrow(() -> new AssertionError("No classifier named top.i"));
		...
		assertEquals(1.0, ((RealValue) etefResult.getValues().get(3)).getValue(), 0.0);
		assertEquals(1.0, ((RealValue) etefResult.getValues().get(4)).getValue(), 0.0);
		...
		assertEquals("Latency results for ete", etefResult.getMessage());
	}

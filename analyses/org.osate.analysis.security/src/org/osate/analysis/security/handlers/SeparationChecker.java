...
public final class SeparationChecker extends AbstractInstanceOrDeclarativeModelReadOnlyHandler {
	...
	public String getMarkerType() {
		return "org.osate.analysis.security.SecurityCheckObjectMarker";
	}
...
//		monitor.done();
	...
	protected void analyzeInstanceModel(IProgressMonitor monitor, AnalysisErrorReporterManager errManager,
		...
		monitor.beginTask(getActionName(), IProgressMonitor.UNKNOWN);
		Optional<SecurityPolicy> policy = SecurityPolicy.of(root);

		if (!policy.isPresent()) {
			error(root, "Missing security policy for " + root.getQualifiedName());
			return;
		}

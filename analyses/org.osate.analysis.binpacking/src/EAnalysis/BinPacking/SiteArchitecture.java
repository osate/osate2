...
public class SiteArchitecture implements Cloneable {
	...
	public Object clone() {
		...
		for (Iterator sites = sitesBySize.iterator(); sites.hasNext();) {
			...
			TreeSet connectivityVector = (TreeSet) siteConnectivityMatrix
					.get(originalSite);
			if (connectivityVector != null) {
				//TreeSet clonedConnVector = new TreeSet(new HostComparator());
				for (Iterator ducts = connectivityVector.iterator(); ducts
						.hasNext();) {
					...
					Duct cloneDuct = new Duct(d.maximumPower, d.maximumSpace,
							d.potentialGuests);
					for (int i = 0; i < d.siteMembers.size(); i++) {
						Site originalPartner = (Site) d.siteMembers.get(i);
						Site clonedPartner = (Site) originalToClone
								.get(originalPartner);
	...
	public void addDuctGuest(HardwareNode l, Duct d) {
		...
		for (int i = 0; i < siteMemberSize; i++) {
			TreeSet siteConnVector = (TreeSet) siteConnectivityMatrix
					.get(d.siteMembers.get(i));

			/* reposition duct based on new size */
			siteConnVector.remove(d);
		}
		boolean b = d.addGuest(l);
		System.out.println("Adding duct guest.space(" + l.spaceRequirement
				+ ") duct(" + d + ").space(" + d.availableSpace + ")");
		for (int i = 0; i < siteMemberSize; i++) {
			TreeSet siteConnVector = (TreeSet) siteConnectivityMatrix
					.get(d.siteMembers.get(i));
	...
	public void addDuct(Duct d) {
		...
		for (int i = 0; i < siteMembersSize; i++) {
			...
			TreeSet site1ConnVector = (TreeSet) siteConnectivityMatrix
					.remove(site1);

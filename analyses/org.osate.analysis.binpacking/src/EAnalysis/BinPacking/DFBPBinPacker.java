...
public class DFBPBinPacker extends BaseLowLevelBinPacker
{
	...
	System.out.println("\n\n *** DFBPBinPacker initial Hardware.size("+problem.hardwareGraph.size()+") validProcessor.size("+validProcessors.size()+") for total bandwidth("+aggregateBandwidth+") ** \n");
...
// 	    }
// 	System.out.println(") ---");
	TreeSet disconnectedComponents = getDisconnectedComponents(new BandwidthComparator(),
								   moduleAggregate,
								   problem);
...
// 	System.out.println("---- END OF DISCONNECTED COMPONENT ---");
	...
	for (Iterator subGraphsList = disconnectedComponents.iterator();
	     subGraphsList.hasNext();)
	    {
		...
		for (Iterator processorList = affinityProcessorList.iterator(); //validProcessors.iterator(); 
			...
			if (processor.canAddToFeasibility(composite))
				...
				System.out.println(" \t\t ++++ ADDED COMPOSITE("+composite.name+") TO PROCESSOR("+processor+").NAME("+processor.name+")");
				/* add messages to neighbors already deployed */
				TreeMap connVector = (TreeMap) problem.softwareConnectivity.get(composite);
				if (connVector != null)
				    {
					for (Iterator neighborMsgs = connVector.entrySet().iterator();
					     neighborMsgs.hasNext();)
					    {
						...
						if (neighbor instanceof CompositeSoftNode)
							...
							if ((neighborProcessor = neighbor.getDeployedTo()) != null)
								...
								if (links != null)
								    {
									for (Iterator linkList = links.iterator();
									     (!foundLink) && linkList.hasNext();)
									    {
										hostLink = (Link) linkList.next();
										System.out.println("\t\t\t --- checking link ("+hostLink+")");
										for (Iterator connNodes = hostLink.getConnectedNodes().iterator();
										     (!foundLink) && connNodes.hasNext();)
										    {
											...
											if (other.equals(neighborProcessor))
												...
												if (hostLink.canAddToFeasibility(msg))
													...
													System.out.println("\t\t\t\t\t msg deployed to link("+hostLink+")!");
													foundLink=true;
												    }
												else
												    {
													System.out.println("\t\t\t\t\t msg,bw("+msg.getBandwidth()+") DOESN'T FIT in link.bw("+hostLink.getAvailableCapacity()+")!");
												    }
											    }
										    }
									    }
			    ...
			    {
				//System.out.println("processor("+processor.name+").avail("+processor.getAvailableCapacity()+").addIfFeasible(module("+composite.name+").size("+composite.getBandwidth()+")) FAILED!");
			    }
		    }
		...
		if (!subGraphsList.hasNext())
			...
			if (subGraphsList.hasNext())
				...
				if (validProcessors.size() >0)
					...
					double partitionSize = expansor.getLargestProcessorSizeForModule(toPartition, validProcessors, problem);
 					System.out.println("trying to partition module("+toPartition.name+") into chunks size("+partitionSize+")");
...
// 					    }
// 					System.out.println("]");
					double cutBandwidth = partition(toPartition, 
									partitionSize, 
...
// 						System.out.println("-----------------");
						...
						System.out.println("\t\t\t toPartition.size = "+toPartition.getBandwidth()+" partitionSize = "+partitionSize);
						largestProcessor[0]=null; largestSite[0]=null;
						if (!expansor.expandProcessorForModule(toPartition, validProcessors, problem, largestProcessor, largestSite))
						    {
							...
							if (largestProcessor[0] != null &&
								...
								cutBandwidth = partition(toPartition, 
											 partitionSize, 
											 disconnectedComponents,
											 problem,
...
// 									System.out.println("-----------------");
									System.out.println("largestProcessor["+largestProcessor[0]+
											   "] largestSite["+largestSite[0]+
											   "] validProcessors["+validProcessors+
											   "], problem["+problem+"]");
									expansor.cloneProcessorInto(largestProcessor[0], largestSite[0], validProcessors, problem);
									disconnectedComponents.remove(toPartition);
									subGraphsList = disconnectedComponents.iterator();
								    }
								else
								...
								if (largestProcessor[0] != null)
								    System.out.println("\t\t\t Available Capacity= "+largestProcessor[0].getAvailableCapacity()+" partitionSize= "+partitionSize);
								return false;
							    }
						    }
						else
				    ...
				    {
					largestProcessor[0]=null; largestSite[0]=null;
					if (!expansor.expandProcessorForModule(composite, validProcessors, problem, largestProcessor, largestSite))
					    {

...
public class RandomBinPacker extends BaseLowLevelBinPacker {
	 ...
	 * be deployed
	 */
	public boolean solve(TreeSet moduleAggregatePrime, TreeSet validProcessors,
			OutDegreeAssignmentProblem problem) {
		Vector v = new Vector();
		v.addAll(moduleAggregatePrime);
		Vector moduleAggregate = new Vector();
		while (v.size() > 0) {
			int index = (int) Math.ceil(randomGenerator.nextDouble()
					* (v.size() - 1));
		...
		for (Iterator subGraphsList = disconnectedComponents.iterator(); subGraphsList
			...
			for (Iterator processorList = affinityProcessorList.iterator(); processorList
					.hasNext();) {

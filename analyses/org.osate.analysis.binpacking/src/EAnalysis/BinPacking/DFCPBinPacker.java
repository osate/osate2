...
public class DFCPBinPacker extends BaseLowLevelBinPacker {
	Expansor expansor;

	public DFCPBinPacker(Expansor e) {
		expansor = e;
	}
	...
	public boolean solve(TreeSet moduleAggregate, TreeSet validProcessors,
		...
		System.out.println("\n\n *** DFCPBinPacker initial Hardware.size("
				+ problem.hardwareGraph.size() + ") validProcessor.size("
				+ validProcessors.size() + ") for total bandwidth("
				+ aggregateBandwidth + ") *** \n");

		for (Iterator iter = problem.hardwareGraph.iterator(); iter.hasNext();) {
			HardwareNode n = (HardwareNode) iter.next();
			if (n.getHost() == null)
				System.out.println("Node(" + n + ").hashCode(" + n.hashCode()
						+ ") host == null");
		}
		...
		for (Iterator subGraphsList = disconnectedComponents.iterator(); subGraphsList
							...
							if (neighbor instanceof CompositeSoftNode) {
								...
								if ((neighborProcessor = neighbor
									...
									if (!foundLink) {
										 ...
										 * it
										 */
										//DebugMonitor.println(DebugMonitor.channels[0],"DFCPBinPacker:
										// Adding link
										// between(node("+processor.toString()+"),
										// node("+neighborProcessor.toString()+"))");
										if (problem.hardwareConnectivity
												.get(neighborProcessor) == null) {
			...
			if (!subGraphsList.hasNext()) {
				...
				if (subGraphsList.hasNext()) {
				    ...
				    // if we have made not progress (deployment) since the last time we partitioned
				    // we are already stuck. Hence fail.
				    if (!progress)
					return false;

				    // mark progress as false. I will go to true if after this partitioning cycle
					...
					if (validProcessors.size() > 0) {
						double partitionSize = expansor
								.getLargestProcessorSizeForModule(composite,
						...
						// 					    }
						//System.out.println("]");
						double cutBandwidth = partition(composite,
								partitionSize, disconnectedComponents, problem,
						...
						if (cutBandwidth >= 0) {
							...
							if (!expansor.expandProcessorForModule(
								...
								if (largestProcessor[0] != null
									 ...
									 * new size
									 */
									cutBandwidth = partition(composite,
											partitionSize,
											disconnectedComponents, problem,
											BY_BANDWIDTH); //BY_SIZE);
									if (cutBandwidth >= 0) {
						...
						if (!expansor.expandProcessorForModule(
								composite, validProcessors,
								problem, largestProcessor, largestSite)) {
							System.out.println("valid processors.size("
									+ validProcessors.size() + ") composite("
									+ composite.name + ").BW("
									+ composite.getBandwidth() + ")");

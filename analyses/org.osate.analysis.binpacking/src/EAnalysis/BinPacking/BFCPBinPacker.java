...
public class BFCPBinPacker extends BaseLowLevelBinPacker {
	...
	public boolean solve(TreeSet moduleAggregate, TreeSet validProcessors,
		...
		expansor.createInitialHardware(problem, validProcessors,
				aggregateBandwidth);
		System.out.println("\n\n *** BFCPBinPacker initial Hardware.size("
		...
		if (validProcessors.size() == 0) {
			for (Iterator iter = moduleAggregate.iterator(); iter.hasNext();) {
				...
				if (n instanceof CompositeSoftNode) {
					System.out.println("- Composite(" + n + ").name(" + n.name
							+ ").deployedTo(" + n.getDeployedTo() + ")");
					for (Iterator iter1 = ((CompositeSoftNode) n)
							.getBasicComponents().iterator(); iter1.hasNext();) {
						SoftwareNode inN = (SoftwareNode) iter1.next();
						System.out.println("---> node(" + inN + ").name("
								+ inN.name + ").deployedTo("
								+ inN.getDeployedTo() + ")");
					}
				} else {
					System.out.println("===> node(" + n + ").name(" + n.name
							+ ").deployedTo(" + n.getDeployedTo() + ")");
				}
			}
		}
		...
		for (Iterator subGraphsList = disconnectedComponents.iterator(); subGraphsList
								...
								if ((neighborProcessor = neighbor
									...
									if (!foundLink) {
										...
										if (hostLink == null) {
											...
											System.out
													.println("\t\t Failed to add new link");
											processor
			...
			if (!deployed) {
				...
				if (validProcessors.size() > 0) {
					...
					double cutBandwidth = partition(composite, partitionSize,
							disconnectedComponents, problem, BY_BANDWIDTH); //BY_SIZE);
					if (cutBandwidth >= 0) {

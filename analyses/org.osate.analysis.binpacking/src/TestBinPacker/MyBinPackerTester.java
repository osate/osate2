...
public class MyBinPackerTester {
	...
	public void createWorstFitSoftwareGraph(
		...
		System.out.println("size[0]("
				+ MyBinPackerTester.decFormat.format(sizes[0])
				+ ") sizeSet1.total(" + MyBinPackerTester.decFormat.format(ss1)
				+ ")");
		System.out.println("size[1]("
				+ MyBinPackerTester.decFormat.format(sizes[1])
				+ ") sizeSet2.total(" + MyBinPackerTester.decFormat.format(ss2)
				+ ")");
		System.out.println("size[2]("
				+ MyBinPackerTester.decFormat.format(sizes[2])
				+ ") sizeSet3.total(" + MyBinPackerTester.decFormat.format(ss3)
				+ ")");
		System.out.println("size[3]("
				+ MyBinPackerTester.decFormat.format(sizes[1] + epsilon)
				+ ") sizeSet4.total(" + MyBinPackerTester.decFormat.format(ss4)
				+ ")");

		System.out.println("sizes[0+1+2]("
				+ MyBinPackerTester.decFormat.format(sizes[0] + sizes[1]
						+ sizes[2]) + ")");
		System.out.println("sizes[3*2+2*2]("
				+ MyBinPackerTester.decFormat.format((2 * (sizes[1] + epsilon))
						+ (2 * sizes[2])) + ")");

		int partitionedProcessors = 0;
		long period = 1000000000l;

		System.out.println("Period(" + MyBinPackerTester.decFormat.format(period)
				+ ")ns");
			...
			for (Iterator iter = result.problem.hardwareGraph.iterator(); iter
				...
				for (Iterator taskSet = n.getTaskSet().iterator(); taskSet
					...
					textArea.append("\t Module "
							...
							+ " "
							+ "C("
							+ Long.toString(m.getCycles())
							+ " cycles),"
							+ "T("
							+ Long.toString(m.getPeriod())
							+ " nanos),"
							+ "D("
							+ Long.toString(m.getDeadline())
							+ " nanos)\n"
							+ "\t\t  BW Load("

...
public class Auxillary {
     ...
     public static int phaseConversion( RuntimeProcess processj, RuntimeProcess processi) {
        ...
        if  ( ( processj.getPhaseOffset() < processi.getPhaseOffset())
              ||
              (processj.getARCID() != processi.getARCID()))
            return processj.getPhaseOffset();
        else
     ...
     public static  double processInARCContribution(double window, double windowOffset,
          RuntimeProcess processj, RuntimeProcess processi) {
        double result;
        result = ( ceiling_0( (window + windowOffset -
                       phaseConversion(processj, processi))/processj.getPeriod())
                    - ceiling_1_0 (windowOffset- phaseConversion(processj, processi )))
                   *(processj.getExecutionTime());
        return result;
     }

     public  static double processOtherThanMySelfContribution( double window,
         double windowOffsetWithinMySelf, double windowOffsetRelativeToMySelf,
         RuntimeProcess mySelf, ARCForArray ARC) {
         ...
         for (int i=0; i< number; i++) {
           ...
           if ( (compareNotMySelf(processInstance, mySelf) != false) &&
              ...
              if ( processInstance.getARCID() == mySelf.getARCID() )
                 result += processInARCContribution(window, windowOffsetWithinMySelf,
                    processInstance, mySelf);
              else
                 result += processInARCContribution(window, windowOffsetRelativeToMySelf,
                    processInstance, mySelf);
           }
         }
     ...
     public static boolean compareNotMySelf(RuntimeProcess processx, RuntimeProcess mySelf) {
           if ( (processx.getARCID() == mySelf.getARCID())
                &&
                (processx.getPeriod() == mySelf.getPeriod())
                &&

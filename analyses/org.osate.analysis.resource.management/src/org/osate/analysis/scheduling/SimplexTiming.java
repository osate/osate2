 ...
 */
 public class SimplexTiming {
    ...
    */
    ...
    public void setExactOrTractable(boolean flag) { exactOrTractable = flag; }
    public boolean getExactOrTractable() { return exactOrTractable; }
	
	public void cleanARCList() { ARCList.removeAllElements(); }
              ...
              if (period == -1)
                 period = processInstance.getPeriod();
                   ...
                   if (period != processInstance.getPeriod())
                        System.err.println(" period discrepency detected in the ARC with id: "
                           + curARC.getARCID());
              }
           }
    ...
    public boolean schedulabilityAnalysis() {
        ...
        for (i=0; i < arrayInstance.size(); i++) {
             ...
             for (j=0; j<ARCElement.getNumberOfProcesses(); j++) {
                  ...
                  System.out.println("process: " + j + " in ARC: " + processInstance.getARCID()
                      + " has priority of : " + processInstance.getPriority() + " and period: " +
            ...
            for (i=0; i < numberOfARCs; i++ ) {
                                 ...
                                 for ( k =0; k < numberOfARCs; k++) {
                                    ...
                                    if ( phaseListNotInMySelfArray.getNumberOfItemsInArray() > 0) {
                                        ...
                                        for (m=0; m < phaseListNotInMySelfArray.getNumberOfItemsInArray(); m++) {
                                             ...
                                             response = Auxillary.processOtherThanMySelfContribution(w1, windowOffsetWithinMySelf,
                                                          windowOffsetRelativeToMySelf, processInstance,
             ...
             for (i=0; i< ARCList.size(); i++) {
                ...
                for (j=0; j<t.getNumberOfProcesses(); j++) {
                    ...
                    System.out.println("process: " + tp.getComponentName()+" in ARC: " + i +
                      " with priority: " + processInstance.getPriority()+
                      " is "+(processInstance.getSchedulability()?"":"not ")+"schedulable" +
                      " with response time (- offset): " + (processInstance.getMaxResponseTime()
                      -processInstance.getPhaseOffset())
                    );
                    if (processInstance.getSchedulability()==false) label = 0;
                }
             }
        }
        else {
    ...
    public DoubleArray phaseConstructionInARC( ARCForArray element,
         ...
         for (i=0; i< numberOfProcesses; i++ ) {
               for (j=i+1; j<numberOfProcesses; j++) {
                   ...
                   if ((java.lang.Math.abs (t1 -t2) < 0.5)
                        && ( !(t1 < Global.NEGATIVE_MAX/2)))
    ...
    public DoubleArray giveMeSList ( Vector arrayInstance, int numberOfARCs,
           RuntimeProcess process, double windowOffsetWithinMySelf) {
         ...
         for (i=0; i<numberOfARCs; i++ ) {
             ...
             for (j=0; j< ARCElement.getNumberOfProcesses(); j++ ) {
                   ...
                   if (processInstance.getPriority() > process.getPriority()){
                       ...
                       //------------------------------------------------------
                        System.out.println("the number of possible phases is:  "
                           + phaseArray.getNumberOfItemsInArray() );
                        for (z=0; z<phaseArray.getNumberOfItemsInArray(); z++)
                           System.out.println("::: " + phaseArray.getItemInArray(z) +"::");
                        counterLimit=(int)java.lang.Math.ceil( (process.getDeadline() +
                                      process.getPeriod())/processInstance.getPeriod() ) + 1;
                      //--------------------------------------------------------
                        for (k=0; i<counterLimit; k++) {
                               for (l=0; l<phaseArray.getNumberOfItemsInArray(); l++) {
                                   tentativePoint=k*processInstance.getPeriod()
                                          -phaseArray.getItemInArray(l)
    ...
    public void sensitivityAnalysis() {
        ...
        for (i=0; i<numberOfARCs; i++) {
             ...
             for (j=0; j<ARCElement.getNumberOfProcesses(); j++) {
                  processInstance = ARCElement.getProcess(j);
                  System.out.println("process: " + j + " in ARC: " + i
                     + " has priority of : " + processInstance.getPriority()
                     + " and period: " + processInstance.getPeriod());
             }
        }
        ...
        for (i=0; i< numberOfARCs; i++ ) {
            ...
            for (j=0; j<ARCElement.getNumberOfProcesses(); j++) {
               ...
               for (l=0; l<phaseArray.getNumberOfItemsInArray(); l++) {
                  ...
                  constructFirstLevelArray(arrayInstance, numberOfARCs,
                         processInstance, instancePhase, counter, firstLevelArray);
                  for (m=0; m< counter; m++) {
        ...
        for (i=0; i< numberOfARCs; i++) {
         ...
         for (i=0; i<ARCList.size(); i++) {
              ...
              for (j=0; j< t.getNumberOfProcesses(); j++) {
                  ...
                  System.out.println(" process: " + j + " in ARC : "  + i
                         + " has sensitivity margin of : " + temp.getSensitivity()
                         + " and response time of: "
                         +( temp.getMaxResponseTime()- temp.getPhaseOffset() )
    ...
    */
    public void constructFirstLevelArray( Vector arrayInstance, int numberOfARCs,
         RuntimeProcess process, double phase, int totalProcess, Vector firstLevelArray) {
         ...
         if (timePoints.getNumberOfItemsInArray() == 0) return;
             ...
             for (i=0; i<numberOfARCs; i++) {
              ...
              for (l=0; l< timePoints.getNumberOfItemsInArray(); l++) {
                 ...
                 for (i=0; i<numberOfARCs; i++) {
                       ...
                       if (processInstance.getARCID() == process.getARCID() ) {
                           summation += Auxillary.processOtherThanMySelfContribution(timePoint, phase,
                                  0, process, ARCElement);
                           ...
                           for (m=0; m<t.getNumberOfItemsInArray(); m++) {
                            ...
                            response= Auxillary.processOtherThanMySelfContribution(timePoint, phase,
                        ...
                        if (Auxillary.compareNotMySelf(process, processInstance) == false) {
                            ...
                            ((DoubleArray)firstLevelArray.elementAt(processPos)).addItemInArray(difference);
                              ...
                              if (processInstance.getPriority() > process.getPriority()){
                                  ...
                                  recoverResponse = Auxillary.processOtherThanMySelfContribution(timePoint, phase, recoverPhase, process, ARCElement);
                                  for (n=0; n < ((DoubleArray) phasePossibleArray.elementAt(ARCPos)).getNumberOfItemsInArray(); n++) {
                                      variedPhase = ( (DoubleArray) phasePossibleArray.elementAt(ARCPos)).getItemInArray(n);
                                      renewResponse = Auxillary.processOtherThanMySelfContribution(timePoint,
                                                           phase, variedPhase, process, ARCElement);

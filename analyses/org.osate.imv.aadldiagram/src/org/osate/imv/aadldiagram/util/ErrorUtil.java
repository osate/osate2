...
public class ErrorUtil {
	...
	public static int getFactorWithPath(ComponentInstance componentSource, ComponentInstance componentTarget,
			int scale) {
		...
		if (!efs.isEmpty()) {
			...
			for (ErrorFlow ef : efs) {
				...
				if ((ef instanceof ErrorSource) || (ef instanceof ErrorPath)) {
					...
					List<PropagationPathEnd> ppes = analysisModel.getAllPropagationDestinationEnds(componentSource,
							outp);
				...
				// if (ef instanceof ErrorSink)
				{
					...
					EList<OutgoingPropagationCondition> additionalPropagations = EMV2Util
							.getAdditionalOutgoingPropagation(componentSource, ep);
					...
					if (additionalPropagations.size() == 0) {
						...
						for (OutgoingPropagationCondition opc : additionalPropagations) {
							...
							List<PropagationPathEnd> ppes = analysisModel
									.getAllPropagationDestinationEnds(componentSource, outp);
...
//					traceErrorPaths(ci,outp,EMV2Util.mapToken(tt,ef),depth+1,entryText+", "+generateEffectText(ci, outp,tt));
	...
	public static int getFactor(ComponentInstance source, ComponentInstance dest) {
		...
		if (((dest.getCategory() == ComponentCategory.DEVICE) && (source.getCategory() == ComponentCategory.PROCESSOR))
				|| ((dest.getCategory() == ComponentCategory.PROCESS)
						&& (source.getCategory() == ComponentCategory.PROCESSOR))) {

...
public final class FindUnusedClassifiersAnalysis {
	...
	private final static class FindUnusedClassifiersJob extends WorkspaceJob {
		...
		public IStatus runInWorkspace(IProgressMonitor monitor) throws CoreException {
			...
			final SubMonitor subMonitor = SubMonitor.convert(monitor, declarationURIs.size());
			for (final URI declURI : declarationURIs) {
				if (subMonitor.isCanceled()) {
					throw new OperationCanceledException();
				}

				final SubMonitor nestedMonitor = subMonitor.split(1);

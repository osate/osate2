...
public class ReinstantiateIAllHandler extends AbstractHandler {
	public ReinstantiateIAllHandler() {
		super();
	}

	@Override
	public Object execute(final ExecutionEvent event) throws ExecutionException {
		final List<IContainer> selectedContainers = new ArrayList<>();
		for (final Object selection : HandlerUtil.getCurrentStructuredSelection(event).toList()) {
			final IContainer container = (IContainer) selection;
			selectedContainers.add(container);
		}

		final String lineSeparator = System.lineSeparator();
		final StringBuilder sb = new StringBuilder("Selected Containers:");
...
//		final Job job = new ReinstantiateAllJob(selectedFiles);
...
//		job.schedule();
	...
	private final class ReinstantiateAllJob extends WorkspaceJob {
		private final List<IFile> aaxlFiles;

		public ReinstantiateAllJob(final List<IFile> files) {
			super("Reinstantiate All");
			aaxlFiles = files;
		}

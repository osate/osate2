...
public class MultiSelectSOMChooserDialog extends Dialog {
	...
	private ListViewer allSomsViewer = null;
	...
	private final List<SystemOperationMode> soms = new ArrayList<>();
	 ...
	 */
	public MultiSelectSOMChooserDialog(final Shell parentShell, final SystemInstance systemInstance, final boolean requireName)
	 ...
	 * @throws IllegalArgumentException Thrown if <code>systemInstance</code> is not modal.
	 */
	public MultiSelectSOMChooserDialog(final Shell parentShell, final SystemInstance systemInstance)
			throws IllegalArgumentException {
	...
	public int openThreadSafe() {
		...
		final IntAnswer obj = new IntAnswer() {
			@Override
			public void run() {
				answer = MultiSelectSOMChooserDialog.this.open();
			}
		};
	 ...
	 */
	public MultiSelectSOMChooserDialog(final Shell parentShell, final SystemInstance systemInstance, final String SOMName,
	...
	protected Control createDialogArea(final Composite parent) {
		...
		// List viewer to show a filtered list of all the system operation modes

		allSomsViewer = new ListViewer(composite);
		layoutData = new GridData(SWT.FILL, SWT.FILL, true, true);
		layoutData.widthHint = VIEWER_WIDTH;
		layoutData.heightHint = VIEWER_HEIGHT;
		allSomsViewer.getControl().setLayoutData(layoutData);

		allSomsViewer.setContentProvider(ArrayContentProvider.getInstance());
		allSomsViewer.setLabelProvider(new LabelProvider() {
			@Override
			public String getText(final Object element) {
				final String rawText = element.toString();
				return rawText.replace("#", ", ");
			}
		});
		allSomsViewer.setInput(systemInstance.getSystemOperationModes());
		allSomsViewer.addSelectionChangedListener(selectionChangedEvent -> {
			addButton.setEnabled(!selectionChangedEvent.getSelection().isEmpty());
		});

		// List viewer to show the selected SOMs
		...
		buttonArea.setLayout(rowLayout);
		addAllButton = new Button(buttonArea, SWT.PUSH);
		...
		listViewer.setInput(soms);
		listViewer.addSelectionChangedListener(selectionChangedEvent -> {
			removeButton.setEnabled(!selectionChangedEvent.getSelection().isEmpty());
		});

		addAllButton.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(final SelectionEvent e) {
				soms.addAll(systemInstance.getSystemOperationModes());
				listViewer.refresh();
			}
		});
		...
		addButton.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(final SelectionEvent e) {
//				final SystemOperationMode currentSOM = getCurrentSOM();
//				if (!soms.contains(currentSOM)) {
//					soms.add(currentSOM);
//					listViewer.refresh();
//				}

				final IStructuredSelection selection = (IStructuredSelection) allSomsViewer.getSelection();
				soms.addAll(selection.toList());
				listViewer.refresh();
		...
		removeButton.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(final SelectionEvent e) {
				...
				soms.removeAll(selection.toList());
				listViewer.refresh();
			}
		});
	...
	private void generateSOM() {
		newSOM = getCurrentSOM();
	}

	private SystemOperationMode getCurrentSOM() {
		/*
	...
	private void configureViewer() {
		...
		viewer.setCellModifier(new ICellModifier() {
			...
			public void modify(Object element, String property, Object value) {
				...
				if (selection != -1) {
					final ComponentInstance componentInstance = (ComponentInstance) ((TreeItem) element).getData();
					if (selection == 0) { // clear the mode
						selectedModes.put(componentInstance, null);
					} else {
						selectedModes.put(componentInstance, componentInstance.getModeInstances().get(selection - 1));
					}
					updateOkEnabled();
					refreshSOMsList();
					viewer.refresh();
...
//		addButton.setEnabled(true);
	...
	private void refreshSOMsList() {
		final List<ModeInstance> modeInstances = selectedModes.values().stream().filter(x -> x != null)
				.collect(Collectors.toList());

		final List<SystemOperationMode> soms = systemInstance.getSystemOperationModesFor(modeInstances);
		allSomsViewer.setInput(soms);
		allSomsViewer.refresh();
	}

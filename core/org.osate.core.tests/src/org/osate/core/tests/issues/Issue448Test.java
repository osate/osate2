...
public class Issue448Test extends XtextTest {
	...
	@Test
	public void issue448() throws Exception {
		AadlPackage pkg = testHelper.parseString(AADL_TEXT);
		var classifiers = pkg.getOwnedPublicSection().getOwnedClassifiers();
		assertTrue("System implementation \"test_sys.impl2\" not found",
				classifiers.stream().anyMatch(classifier -> "test_sys.impl2".equals(classifier.getName())));
		SystemImplementation sysImpl = (SystemImplementation) classifiers.stream()
				.filter(classifier -> "test_sys.impl2".equals(classifier.getName()))
				.findFirst()
				.get();
		var instance = InstantiateModel.instantiate(sysImpl);
		assertEquals("test_sys_impl2_Instance", instance.getName());
		var connections = instance.getConnectionInstances();
		assertTrue("In test_sys_impl2_Instance: Expected 5 connections but found " + connections.size(),
				connections.size() == 5);
	}
}

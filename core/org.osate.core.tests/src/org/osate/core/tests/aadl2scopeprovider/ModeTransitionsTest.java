...
public class ModeTransitionsTest extends XtextTest {
	/*
	 ...
	 */
	private static final String MODE_TRANSITION_SCOPE_TEST = """
	...
	private static final List<String> CALL_TRIGGER_PORTS = List.of("af4", "ep4", "es3", "pp3");
	private static final List<String> SUBCOMPONENT_TRIGGER_PORTS = List.of("af2", "ep2", "es2", "pp2");
	private static final List<String> FEATURE_GROUP_TRIGGER_PORTS = List.of("af3", "ep3");
	private static final String DATA_PORT_TRIGGER_ERROR = "Anything in a 'data port' is not a valid mode transition trigger.";
	...
	/*
	 * Tests scope_ModeTransition_source, scope_ModeTransition_destination, scope_ModeTransitionTrigger_context, and
	 * scope_ModeTransitionTrigger_triggerPort
	 */
	@Test
	public void testModeTransitions() throws Exception {
		// Variable issues must be initialized for call to assertConstraints()
		testFileResult = issues = testHelper.testString(MODE_TRANSITION_SCOPE_TEST);
		...
		assertEquals("ModeTransitionScopeTest", pkg.getName());
		var implementation = (AbstractImplementation) pkg.getPublicSection().getOwnedClassifiers().get(1);
		var transition = implementation.getOwnedModeTransitions().getFirst();
		assertEquals("mt1", transition.getName());

		// Tests scope_ModeTransition_source
		assertHelper.assertScope(transition, Aadl2Package.eINSTANCE.getModeTransition_Source(), List.of("m1", "m2"));
		// Tests scope_ModeTransitionTrigger_context
		assertHelper.assertScope(transition, Aadl2Package.eINSTANCE.getModeTransitionTrigger_Context(),
				List.of("asub1", "asub2", "call1", "call12", "call13", "call2", "call3", "call4", "call5", "call6",
						"call7", "call8", "dp1", "dp2", "fg1", "fg2", "subpa1", "subpa2", "subpgsub1", "subpgsub2",
						"subpgsub3", "subpsub1", "subpsub2"));
		// Tests scope_ModeTransitionTrigger_triggerPort(ModeTransition, EReference)
		var implementationTriggerPorts = List.of("af1", "dp1", "dp2", "ep1", "es1", "pp1");
		assertHelper.assertScope(transition, Aadl2Package.eINSTANCE.getModeTransitionTrigger_TriggerPort(),
				implementationTriggerPorts);

		var triggers = transition.getOwnedTriggers();
		assertTrigger(triggers.get(0), null, "af1", implementationTriggerPorts);
		assertTrigger(triggers.get(1), "fg1", "af3", FEATURE_GROUP_TRIGGER_PORTS);
		assertTrigger(triggers.get(2), "fg2", "af3", FEATURE_GROUP_TRIGGER_PORTS);
		AssertHelper.assertError(triggers.get(3), testFileResult.getIssues(), issueCollection,
				DATA_PORT_TRIGGER_ERROR);
		assertTrigger(triggers.get(3), "dp1", "af2", SUBCOMPONENT_TRIGGER_PORTS);
		AssertHelper.assertError(triggers.get(4), testFileResult.getIssues(), issueCollection,
				DATA_PORT_TRIGGER_ERROR);
		assertTrigger(triggers.get(4), "dp2", "af2", SUBCOMPONENT_TRIGGER_PORTS);
		assertTrigger(triggers.get(5), "asub1", "af2", SUBCOMPONENT_TRIGGER_PORTS);
		assertTrigger(triggers.get(6), "asub2", "af2", SUBCOMPONENT_TRIGGER_PORTS);

		var callContexts = List.of("call1", "call2", "call3", "call4", "call5", "call6", "call7", "call8", "call12",
				"call13");
		for (var i = 0; i < callContexts.size(); i++) {
			assertTrigger(triggers.get(7 + i), callContexts.get(i), "af4", CALL_TRIGGER_PORTS);
		}

		// Tests scope_ModeTransition_destination
		assertHelper.assertScope(transition, Aadl2Package.eINSTANCE.getModeTransition_Destination(),
				List.of("m1", "m2"));
	...
	private void assertTrigger(ModeTransitionTrigger trigger, String contextName, String triggerPortName,
			List<String> expectedTriggerPorts) {
		if (contextName == null) {
			assertNull(trigger.getContext());
		} else {
			assertEquals(contextName, trigger.getContext().getName());
		}
		assertEquals(triggerPortName, trigger.getTriggerPort().getName());
		assertHelper.assertScope(trigger, Aadl2Package.eINSTANCE.getModeTransitionTrigger_TriggerPort(),
				expectedTriggerPorts);
	}
}

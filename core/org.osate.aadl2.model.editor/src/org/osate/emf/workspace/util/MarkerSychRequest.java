 ...
 */
class MarkerSynchRequest extends SynchRequest {
	 ...
	 */
	MarkerSynchRequest(WorkspaceSynchronizer synch, Resource resource, IFile file) {
	...
	protected void doPerform() {
		if (!synch.getDelegate().handleResourceMarkersChanged(resource, file)) {
			// note that if our delegate is the default, it
			// will always return true
			WorkspaceSynchronizer.defaultDelegate.handleResourceMarkersChanged(resource, file);
		}
	}
}

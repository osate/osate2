 ...
 * 
 */
class Aadl2ScopeProvider extends PropertiesScopeProvider {
	...
	def scope_SubprogramCall_context(Element context, EReference reference) {
		...
		context.getContainerOfType(BehavioredImplementation)?.members?.filter(CallContext)?.filterRefined?.
	...
	def scope_SubprogramCall_calledSubprogram(Element context, EReference reference) {
		...
		if (callContext === null) {
			...
			scope = context.getContainerOfType(Classifier).members.filter(CalledSubprogram).filterRefined.
				scopeFor(scope)
		} else {
			...
			switch (callContext) {
				ComponentType: {
					...
					val packageClassifiers = new ArrayList(
						callContext.getContainerOfType(AadlPackage).publicSection.ownedClassifiers)
					...
					if (packageSectionForComponentType instanceof PrivatePackageSection &&
						packageSectionForComponentType == context.getContainerOfType(PrivatePackageSection)) {
						packageClassifiers.addAll(packageSectionForComponentType.ownedClassifiers)
					}
					scope = packageClassifiers.filter(CalledSubprogram).filter(ComponentImplementation).filter [
						type == callContext
					].scopeFor(
						[QualifiedName::create(name.substring(name.lastIndexOf('.') + 1))],
						IScope::NULLSCOPE
					)
				...
				SubprogramGroupAccess:
					if (callContext.kind == AccessType::REQUIRES &&
						callContext.subprogramGroupFeatureClassifier instanceof Classifier) {
	...
	def scope_FeatureGroupPrototypeActual_featureType(Element context, EReference reference) {
		context.getContainerOfType(Classifier).allPrototypes.filter(FeatureGroupPrototype).filterRefined.scopeFor(
			scope_Classifier(context, reference))
	}
	...
	def scope_ComponentPrototypeActual_subcomponentType(Element context, EReference reference) {
		context.getContainerOfType(Classifier).allPrototypes.filter(SubcomponentType).filterRefined.scopeFor(
			scope_Classifier(context, reference))
	}
	...
	def scope_AbstractSubcomponent_abstractSubcomponentType(Element context, EReference reference) {
		context.getContainerOfType(ComponentImplementation).allPrototypes.filter(AbstractSubcomponentType).
			filterRefined.scopeFor(scope_Classifier(context, reference))
	}
	...
	def scope_SystemSubcomponent_systemSubcomponentType(Element context, EReference reference) {
		context.getContainerOfType(ComponentImplementation).allPrototypes.filter(SystemSubcomponentType).filterRefined.
			scopeFor(scope_Classifier(context, reference))
	}
	...
	def scope_ProcessSubcomponent_processSubcomponentType(Element context, EReference reference) {
		context.getContainerOfType(ComponentImplementation).allPrototypes.filter(ProcessSubcomponentType).filterRefined.
			scopeFor(scope_Classifier(context, reference))
	}
	...
	def scope_ThreadGroupSubcomponent_threadGroupSubcomponentType(Element context, EReference reference) {
		context.getContainerOfType(ComponentImplementation).allPrototypes.filter(ThreadGroupSubcomponentType).
			filterRefined.scopeFor(scope_Classifier(context, reference))
	}
	...
	def scope_ThreadSubcomponent_threadSubcomponentType(Element context, EReference reference) {
		context.getContainerOfType(ComponentImplementation).allPrototypes.filter(ThreadSubcomponentType).filterRefined.
			scopeFor(scope_Classifier(context, reference))
	}
	...
	def scope_SubprogramSubcomponent_subprogramSubcomponentType(Element context, EReference reference) {
		context.getContainerOfType(ComponentImplementation).allPrototypes.filter(SubprogramSubcomponentType).
			filterRefined.scopeFor(scope_Classifier(context, reference))
	}
	...
	def scope_SubprogramGroupSubcomponent_subprogramGroupSubcomponentType(Element context, EReference reference) {
		context.getContainerOfType(ComponentImplementation).allPrototypes.filter(SubprogramGroupSubcomponentType).
			filterRefined.scopeFor(scope_Classifier(context, reference))
	}
	...
	def scope_ProcessorSubcomponent_processorSubcomponentType(Element context, EReference reference) {
		context.getContainerOfType(ComponentImplementation).allPrototypes.filter(ProcessorSubcomponentType).
			filterRefined.scopeFor(scope_Classifier(context, reference))
	}
	...
	def scope_VirtualProcessorSubcomponent_virtualProcessorSubcomponentType(Element context, EReference reference) {
		context.getContainerOfType(ComponentImplementation).allPrototypes.filter(VirtualProcessorSubcomponentType).
			filterRefined.scopeFor(scope_Classifier(context, reference))
	}
	...
	def scope_DeviceSubcomponent_deviceSubcomponentType(Element context, EReference reference) {
		context.getContainerOfType(ComponentImplementation).allPrototypes.filter(DeviceSubcomponentType).filterRefined.
			scopeFor(scope_Classifier(context, reference))
	}
	...
	def scope_MemorySubcomponent_memorySubcomponentType(Element context, EReference reference) {
		context.getContainerOfType(ComponentImplementation).allPrototypes.filter(MemorySubcomponentType).filterRefined.
			scopeFor(scope_Classifier(context, reference))
	}
	...
	def scope_BusSubcomponent_busSubcomponentType(Element context, EReference reference) {
		context.getContainerOfType(ComponentImplementation).allPrototypes.filter(BusSubcomponentType).filterRefined.
			scopeFor(scope_Classifier(context, reference))
	}
	...
	def scope_VirtualBusSubcomponent_virtualBusSubcomponentType(Element context, EReference reference) {
		context.getContainerOfType(ComponentImplementation).allPrototypes.filter(VirtualBusSubcomponentType).
			filterRefined.scopeFor(scope_Classifier(context, reference))
	}
	...
	def scope_DataSubcomponent_dataSubcomponentType(Element context, EReference reference) {
		context.getContainerOfType(ComponentImplementation).allPrototypes.filter(DataSubcomponentType).filterRefined.
			scopeFor(scope_Classifier(context, reference))
	}
	...
	def scope_DataPort_dataFeatureClassifier(Element context, EReference reference) {
		context.getContainerOfType(Classifier).allPrototypes.filter(DataSubcomponentType).filterRefined.scopeFor(
			scope_Classifier(context, reference))
	}
	...
	def scope_FeatureGroup_featureType(Element context, EReference reference) {
		context.getContainerOfType(Classifier).allPrototypes.filter(FeatureType).filterRefined.scopeFor(
			scope_Classifier(context, reference))
	}
	...
	def scope_SubprogramAccess_subprogramFeatureClassifier(Element context, EReference reference) {
		context.getContainerOfType(Classifier).allPrototypes.filter(SubprogramSubcomponentType).filterRefined.scopeFor(
			scope_Classifier(context, reference))
	}
	...
	def scope_SubprogramGroupAccess_subprogramGroupFeatureClassifier(Element context, EReference reference) {
		context.getContainerOfType(Classifier).allPrototypes.filter(SubprogramGroupSubcomponentType).filterRefined.
			scopeFor(scope_Classifier(context, reference))
	}

	// Reference is from BusAccess in Aadl2.xtext
	def scope_BusAccess_busFeatureClassifier(Element context, EReference reference) {
		context.getContainerOfType(Classifier).allPrototypes.filter(BusFeatureClassifier).filterRefined.scopeFor(
			scope_Classifier(context, reference))
	}
	...
	def scope_FlowSegment_flowElement(FlowSegment context, EReference reference) {
		context.context?.scopeForElementsOfContext(context.getContainerOfType(Classifier), [
			allFlowElements.filterRefined
		]) ?: scope_FlowSegment_flowElement(context.owner as FlowImplementation, reference)
	}
	...
	def scope_EndToEndFlowSegment_flowElement(EndToEndFlowSegment context, EReference reference) {
		context.context?.scopeForElementsOfContext(context.getContainerOfType(Classifier), [
			allEndToEndFlowElements.filterRefined
	...
	def scope_NumberType_referencedUnitsType(Element context, EReference reference) {
		new SimpleScope(delegateGetScope(context, reference).allElements.filter [
			name == qualifiedName || AadlUtil::isPredeclaredPropertySet(qualifiedName.firstSegment)
		], true)
	}
	...
	def private static findClassifierForComponentPrototype(ComponentPrototype prototype,
		Classifier containingClassifier) {
	...
	def private static findClassifierForComponentPrototype(ComponentPrototype prototype,
		...
		PropertiesLinkingService.findClassifierForComponentPrototype(classifierPrototypeContext,
								...
								switch callContext : context.context {
									...
									FeatureGroup: {
										val prototypeContext = switch callContextFeatureType : callContext.allFeatureType {
											...
											FeatureGroupPrototype:
												callContextFeatureType.
													findFeatureGroupTypeForFeatureGroupPrototype(containingClassifier)
										}
										if (prototypeContext !== null) {
											accessFeatureClassifier.
												findClassifierForComponentPrototype(prototypeContext)
										}
									}
									SubprogramGroupAccess: {
										val prototypeContext = switch callContextFeatureClassifier : callContext.allFeatureClassifier {
											...
											ComponentPrototype:
												callContextFeatureClassifier.
													findClassifierForComponentPrototype(containingClassifier)
										}
										if (prototypeContext !== null) {
											accessFeatureClassifier.
												findClassifierForComponentPrototype(prototypeContext)
									...
									SubprogramGroupSubcomponent: {
										...
										while (callContextSubcomponent.subcomponentType === null &&
											callContextSubcomponent.refined !== null) {
										...
										switch callContextSubcomponentType : callContextSubcomponent.subcomponentType {
											ComponentClassifier: {
												if (callContextSubcomponent.ownedPrototypeBindings.empty) {
													accessFeatureClassifier.
														findClassifierForComponentPrototype(callContextSubcomponentType)
												} else {
													accessFeatureClassifier.
														findClassifierForComponentPrototype(callContextSubcomponentType,
											...
											ComponentPrototype: {
												var prototypeContext = callContextSubcomponentType.
													findClassifierForComponentPrototype(containingClassifier)
												if (prototypeContext !== null) {
													accessFeatureClassifier.
														findClassifierForComponentPrototype(prototypeContext)

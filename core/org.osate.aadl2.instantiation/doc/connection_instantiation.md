...
| **8** | Diagnostic set | warning `Could not continue connection from <src>  through <dst>. No connection instance created.` (two spaces) for a path travelling up that the level above does not continue | no diagnostic | Issue #3050 reviewed this behavior and keeps it silent. An uncontinued upward declaration and the symmetric downward declaration from an unconnected boundary are both seedless path fragments; neither forms a semantic connection. Reporting only the upward case would make traversal direction determine diagnostics. Missing continuations are instead reported when a materialized connection reaches or starts at a boundary, as described by entry 5. |
...
| [#3050](https://github.com/osate/osate2/issues/3050) | Report an upward path the level above does not continue, or decide not to | §9 — allowlist entry 8 |
